...
export function logout(str) {
	locache.remove('loginRefresh');
	locache.remove('XToken');
	locache.remove('product');
}
...
window.setProduct = function( product){
	locache.set("product",product);
}
...
export function onSessions (sessions) {
  // 如果会话对象不是好友，需要更新好友名片
  store.dispatch("updateSessionAccount",sessions);
  store.commit(type.UPDATE_DATE_SESSIONS, sessions)
}
...
export function encode(_map, _content) {
  _content = '' + _content
  if (!_map || !_content) {
    return _content||''
  }
...
export function escape( _content){
  let _reg = /<br\/?>$/
  let _map = {
    r:/\<|\>|\&|\r|\n|\s|\'|\"/g,
    '<':'&lt;',
  ...
  return encode(_map, _content).replace(_reg, '<br/>');
}
...
export function generateChatroomSysMsg(data) {
  let text
...
export function scrollChatListDown(pos, initCount) {
    let dom = document.getElementById('msgDtls_main')
    if (!dom) {
      return
    }
    let maxCount = 5;
...
export function onMsg (msg) {
  	store.commit('PUT_MSG', msg)
    store.commit('UPDATE_CURR_SESSION_MSGS', {
      type: 'put',
      msg
   	})
    // 发送已读回执
...
export function onUpdateFriend(error, friends) {
  ...
  store.dispatch('searchUsers', {
    accounts: friends.map(item => {
      ...
      const nim = store.state.initNimSDK.nim
      friends = nim.mergeFriends(friends, users)
      // 更新好友列表
      store.commit('UPDATE_FRIENDS', friends)
      // 更新好友资料
      store.commit('UPDATE_USER_INFO', friends)
    }
  })
}
