			...
			<div id="toComment">
				...
				<div class="toComment" ref="toComment">
					<div class="flex toCt_btn">
			    	 ...
			    	 <div class="replyWrap">
					     <textarea ref="textReply" class="replyTextArea" @blur="bottom('blur')" @focus="bottom('focus')" maxlength="200" v-model="reply_value" :placeholder="'回复'+reply_place" @input="replyItems()">
					      </textarea>
				     </div>
			    </div>
			</div>
...
export default {
	...
	watch: {
		getToComment( e){
			...
			if(!this.isnull(e.data)){
				this.dataArr = e;
				this.reply_place = !this.isnull(e.x) ? e.x.createByName : (e.data.createByName || "楼主");
			}else{
				this.reply_place = "楼主";
			}
		}
	...
	methods: {
		...
		bottom( m){
			...
			/*let _this=this;
			setTimeout(function(){
				console.log(_this.$refs.textReply);
	           _this.$refs.textReply.scrollIntoView(true);
	         },100);*/
		},
	}
}
